...
// Producer
DEFINE_string(image_dir, "examples/media/alicia_images_hands/", 
    "Process a directory of images. Read all standard formats (jpg, png, bmp, etc.).");
DEFINE_string(calibration_dir, "models/cameraParameters/flir/", 
    "Calibration directory with xml files");
...
public:
    ...
    {
        for (int i = 0; i < 2; i++) {
            mCams.emplace_back(std::make_shared<op::WebcamReader>(i));
        }
    ...
    std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>> workProducer()
    {
        try
        {
            ...
            {
                std::cout << "mImageFiles.size(): " << mImageFiles.size() << std::endl;
            ...
            else
            {
                if (mBlocked.empty()) {
                    for (size_t i = 0; i < 2; i++) {
                        ...
                        /*datumPtr->cameraIntrinsics = OP_CV2OPMAT(cv::Mat(3, 4, CV_8UC1));
                        datumPtr->cameraExtrinsics = OP_CV2OPMAT(cv::Mat(3, 4, CV_8UC1));
                        datumPtr->cameraMatrix = OP_CV2OPMAT(cv::Mat(3, 4, CV_8UC1));*/
                        datumPtr->cameraIntrinsics = mIntrinsics[i];
                        //std::cout << "datumPtr->cameraIntrinsics: " << std::endl << " " << datumPtr->cameraIntrinsics.getCvMat() << std::endl << std::endl;
                        //std::cout << "datumPtr->cameraIntrinsics: " << (datumPtr->cameraIntrinsics).data() << std::endl;
                        //cv::Mat M = (datumPtr->cameraIntrinsics).getCvMat();
                        //for (int x = 0; x < datumPtr->cameraIntrinsics.rows() ; x++) {
                        //    for (int y = 0; y < datumPtr->cameraIntrinsics.cols() ; y++) {
                        //        std::cout << (datumPtr->cameraIntrinsics).;  // display the current element out of the array
                        //    }
                        //    std::cout << std::endl;  // when the inner loop is done, go to a new line
                        //}

                        datumPtr->cameraExtrinsics = mExtrinsics[i];
...
private:
    op::CameraParameterReader mParamReader;
