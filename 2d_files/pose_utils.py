...
def conf_select(data, xyc_select, threshold=0.75):
    ...
    return all_data.index, all_data[['x', 'y']], all_data['c']
...
def convert_openpose_csv_frame(csv_data, idx):
    chunked_list = []
    cols_list = ['Body', 'L Hand', 'R Hand']
    for i, col_name in enumerate(cols_list):
        print("idx: ", idx)
        for i in range(1185):
            print(csv_data.index[idx])
        # print("csv_data.index[idx]: ", csv_data.index[idx])
        # print("col_name: ", col_name)
        # print("csv_data.loc[csv_data.index[idx], col_name]: ", csv_data.loc[csv_data.index[idx], col_name])
        points_list = list(csv_data.loc[csv_data.index[idx], col_name])
        if col_name == 'Body':
            chunked_list = chunked_list \
                           + [points_list[i:i + 3]
                              for i in range(0, len(points_list) - 3, 3)] #skip the "background" key points for now
        else:
            chunked_list = chunked_list \
                          + [points_list[i:i + 3]
